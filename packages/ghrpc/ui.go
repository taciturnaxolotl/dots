...
// labelWidth is the widest label this run will print. Every row pads to it, so
// values line up in one column from the first line to the last.
var labelWidth = 7

// alignLabels sizes the label column to the longest label we know we may
// print, before anything is printed.
func alignLabels(labels ...string) {
...
func step(title string, action func()) {
	...
	// The spinner sits in the label column and its text starts where every
	// other value does, so a slow step lines up with the rows around it.
	_ = spinner.New().
		Type(spinner.Dots).
		Title(strings.Repeat(" ", labelWidth) + title).
