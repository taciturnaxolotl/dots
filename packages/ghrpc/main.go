...
func run(cfg config, opts *options, set given) error {
	...
	// Size the label column once, before the first line is printed.
	labels := []string{"init", "tangled", "github", "origin", "visibility", "warning", "error"}
	for _, licence := range licenceRows(opts) {
		labels = append(labels, licence[0])
	}
	alignLabels(labels...)
...
// licenceRows pairs each licence with the slots it covers. Slots that landed
// on the same licence share a line: "firmware/docs".
func licenceRows(opts *options) [][2]string {
	var rows [][2]string
	...
	for i := 0; i < len(slots); {
		...
		rows = append(rows, [2]string{strings.Join(keys, "/"), licenseRegistry[id].Name})
		i = j
	}
	return rows
}
