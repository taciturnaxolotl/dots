...
in
{
  ...
  config.users.groups = mapInstances (
    name: instanceCfg: {
      "emojibot-${name}" = { };
    }
  );

  config.systemd.services = mapInstances (
    name: instanceCfg: {
      "emojibot-${name}" = {
        description = "Emojibot for ${
          if instanceCfg.workspace != null then instanceCfg.workspace else name
        }";
  ...
  config.services.caddy.virtualHosts = mapInstances (
    name: instanceCfg: {
      ${instanceCfg.domain} = {
        extraConfig = ''
          tls {
  ...
  config.security.sudo.extraRules = flatten (
    mapAttrsToList (
      name: instanceCfg:
      optionals instanceCfg.enable [
        {
