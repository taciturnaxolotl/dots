...
REPO_NAME=\$(\$SQLITE3 "\$DB_PATH" "SELECT repo_name FROM repo_keys WHERE repo_did = '\$REPO_DID' AND owner_did = '\$OWNER_DID';" 2>/dev/null)

if [ -z "\$REPO_NAME" ]; then
    echo "\$(date +'%Y-%m-%d %H:%M:%S'): Skipping \$REPO_DID (not found in DB or not owned by \$OWNER_DID)" >> "\$LOG_FILE"
    exit 0
fi

echo "\$(date +'%Y-%m-%d %H:%M:%S'): Resolved \$REPO_DID -> \$REPO_NAME" >> "\$LOG_FILE"
