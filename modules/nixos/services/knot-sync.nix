...
let
  # The template is fully resolved at eval time, so it belongs in the store.
  # It used to be written to a fixed path in shared /tmp and read back, which a
  # local user could pre-create as a symlink to land an arbitrary post-receive
  # hook in every mirrored repo.
  hookTemplate = pkgs.writeShellScript "knot-post-receive" ''
    #!/usr/bin/env bash
